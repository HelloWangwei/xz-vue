<template>
    <div id="swiper" @mouseover="stopAuto" @mouseout="autoPlay">
        <div class="swiper-box" :style="{width:allCount,'-webkit-transition':transitionConfig,'-webkit-transform':slateX}" ref='swiper'>
            ...
            <swiper-item v-for="(item,index) in imgList" :img="item.img" :imgWidth="imgWidth*index" :key="item"/>
            <swiper-item :img="imgList[0].img" :imgWidth="imgWidth*imgList.length" v-if="loop"/>
        </div>
        <span class="glyphicon glyphicon-remove-circle closeBtn" @click="closeSwiper"></span>
        <span class='swiper-left glyphicon glyphicon-chevron-left' @click="toLeft"></span>
        <span class='swiper-right glyphicon glyphicon-chevron-right' @click='toRight'></span>
        <div class="swiper-dots" v-if="dots">
            <button v-for="item in imgList" :class="{active:index==dotsIndex}" @click="toDots(index)">{{item}}</button>
        </div>
    </div>
</template>
